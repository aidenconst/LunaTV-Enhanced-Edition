...
function getSearchCache(query: string): SearchCache | null {
...
function SearchPageClient() {
  ...
  // 新增：获取缓存（放在 trimmedQuery 定义之后，useQuery 之前）
  const cached = trimmedQuery ? getSearchCache(trimmedQuery) : null;
  ...
  const streamedSearchQuery = useQuery<StreamedState>({
    ...
    staleTime: Infinity, // 改为 Infinity
    gcTime: 0, // 保持 0 或也可设为 Infinity，不影响
    refetchOnMount: false, // 新增
    refetchOnWindowFocus: false, // 新增
    initialData: cached
      ? {
          results: cached.results,
          totalSources: cached.totalSources,
          completedSources: cached.completedSources,
        }
      : undefined,
  });
  ...
  useEffect(() => {
    if (
      useFluidSearch &&
      streamedSearchQuery.data &&
      streamedSearchQuery.data.completedSources ===
        streamedSearchQuery.data.totalSources &&
      streamedSearchQuery.data.totalSources > 0 &&
      trimmedQuery
    ) {
      setSearchCache(
  ...
  }, [useFluidSearch, streamedSearchQuery.data, trimmedQuery]);

  // ========== 新增：传统搜索完成时缓存 ==========
  useEffect(() => {
    if (!useFluidSearch && traditionalSearchQuery.data && trimmedQuery) {
      setSearchCache(trimmedQuery, traditionalSearchQuery.data, 1, 1);
    }
  }, [useFluidSearch, traditionalSearchQuery.data, trimmedQuery]);
  // 构建筛选选项
        ...
        <div className='mb-8'>
          ...
          <div className='max-w-3xl mx-auto mb-6 px-3 sm:px-0'>
            ...
            <div className='overflow-x-auto scrollbar-hide -mx-3 px-3 sm:mx-0 sm:px-0'>
              <div className='inline-flex sm:flex items-center justify-start sm:justify-center min-w-full sm:min-w-0 gap-1 sm:gap-2 rounded-2xl p-1 backdrop-blur-sm  '>
                {/* 影视资源 */}
                <button
                  type='button'
                  onClick={() => {
                  ...
                  className={`flex-shrink-0 px-4 sm:px-6 py-2.5 text-sm sm:text-base font-medium rounded-xl transition-all duration-200 whitespace-nowrap ${
                    searchType === 'video'
                      ? 'bg-white dark:bg-gray-900 text-green-600 dark:text-green-400 shadow-sm ring-1 ring-green-500/30'
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-4 sm:px-6 py-2.5 text-sm sm:text-base font-medium rounded-xl transition-all duration-200 whitespace-nowrap ${
                    searchType === 'netdisk'
                      ? 'bg-white dark:bg-gray-900 text-blue-600 dark:text-blue-400 shadow-sm ring-1 ring-blue-500/30'
                      : 'text-gray-700 dark:text-gray-300 hover:bg-white/70 dark:hover:bg-gray-900/70'
                  }`}
                >
                  💾 网盘资源
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-4 sm:px-6 py-2.5 text-sm sm:text-base font-medium rounded-xl transition-all duration-200 whitespace-nowrap ${
                    searchType === 'youtube'
                      ? 'bg-white dark:bg-gray-900 text-red-600 dark:text-red-400 shadow-sm ring-1 ring-red-500/30'
                      : 'text-gray-700 dark:text-gray-300 hover:bg-white/70 dark:hover:bg-gray-900/70'
                  }`}
                >
                  📺 YouTube
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-4 sm:px-6 py-2.5 text-sm sm:text-base font-medium rounded-xl transition-all duration-200 whitespace-nowrap ${
                    searchType === 'bilibili'
                      ? 'bg-white dark:bg-gray-900 text-pink-600 dark:text-pink-400 shadow-sm ring-1 ring-pink-500/30'
                      : 'text-gray-700 dark:text-gray-300 hover:bg-white/70 dark:hover:bg-gray-900/70'
                  }`}
                >
                  📺 Bilibili
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-4 sm:px-6 py-2.5 text-sm sm:text-base font-medium rounded-xl transition-all duration-200 whitespace-nowrap ${
                    searchType === 'tmdb-actor'
                      ? 'bg-white dark:bg-gray-900 text-purple-600 dark:text-purple-400 shadow-sm ring-1 ring-purple-500/30'
                      : 'text-gray-700 dark:text-gray-300 hover:bg-white/70 dark:hover:bg-gray-900/70'
                  }`}
                >
                  🎬 TMDB演员
