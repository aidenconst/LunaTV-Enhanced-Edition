...
function HomeClient({
  ...
  // 安全读取 window：在服务器渲染时返回默认值，组件挂载后再从 window 读取真实值
  const [enableVideo, setEnableVideo] = useState<boolean>(() => {
    if (typeof window === 'undefined') return true; // SSR or before mount -> 默认开启视频
    return !(window as any).RUNTIME_CONFIG?.DISABLE_HERO_TRAILER;
  });

  useEffect(() => {
    if (typeof window === 'undefined') return;
    setEnableVideo(!(window as any).RUNTIME_CONFIG?.DISABLE_HERO_TRAILER);
  }, []); // 仅在客户端挂载时读取一次
